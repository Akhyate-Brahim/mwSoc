...
public class AutoAck implements javax.jms.MessageListener{
	...
	public void onMessage(Message message) {
		...
		// via la queue
		try {
			// on remarque que on peut faire un getString("num"), parce qu'ici, le resultat certes un entier, est utilise en tant que string dans le print
			System.out.print("Recu un message de la queue: "+((MapMessage)message).getString("nom") + " de num = "+ ((MapMessage)message).getString("num")+" ");
			int num=((MapMessage)message).getInt("num"); // par contre , ici on doit faire un getInt puisque on veut vraiment raisonner sur la valeur de l'entier
			System.out.print(num); 
			System.out.println(" Mess redelivre: "+message.getJMSRedelivered());
			if ((num%4)==0){
				System.out.println("Simulons un probleme durant l'execution de onMessage empechant son execution complete");
				throw new RuntimeException("prbm du MOM!!");
			}
		} catch (JMSException e){ 
		// TODO Auto-generated catch block
		e.printStackTrace();
